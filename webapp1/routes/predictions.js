...
var salemSchedule = {table: [] };
					...
					for (var j=0; j<mode['route'].length; j++){
						...
						for (var k=0; k<route['direction'].length; k++){
							...
							for (var l=0; l<direction['trip'].length; l++){
								...
								salemSchedule.table.push({train_name: direction['trip'][l]['trip_name'], scheduleTime: direction['trip'][l]['sch_arr_dt'],predictedTime:direction['trip'][l]['pre_dt']});
	...
	Promise.all([salemPromise]).then(values => { 
	  res.render('predictions', {title:'preditions' , salemSchedule:salemSchedule });
